# Contributing to `tree-sitter-lua-spm`

## Scope

This repository is a packaging wrapper, not a product. It exists so Swift
projects can depend on the upstream Lua tree-sitter grammar without running
...
[`tree-sitter-lua`](https://github.com/tree-sitter-grammars/tree-sitter-lua) tree-sitter grammar. The generated parser
(`Sources/TreeSitterLua/src/parser.c` and any `scanner.c`) is **vendored** so
SPM can build it without `tree-sitter-cli`. This grammar ships an external `scanner.c`.
...
You should see `T _tree_sitter_lua`. If the grammar ships an
...
2. Copy `src/parser.c`, `src/scanner.c` (if present), and `src/tree_sitter/*.h`
   into `Sources/TreeSitterLua/src/`.
